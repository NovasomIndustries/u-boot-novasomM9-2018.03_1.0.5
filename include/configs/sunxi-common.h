...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"image=Image\0" \
        "initrd=uInitrd\0" \
        "fdt_file=novasom_m9.dtb\0" \
        "mmcloadimage=fatload mmc ${mmcdev}:${mmcpart} ${kernel_addr_r} ${image}\0" \
        "mmcloadinitrd=fatload mmc ${mmcdev}:${mmcpart} ${ramdisk_addr_r} ${initrd}\0" \
        "mmcloadfdt=fatload mmc ${mmcdev}:${mmcpart} ${fdt_addr_r} ${fdt_file}\0" \
        "mmcloadbootenv=fatload mmc ${mmcdev}:${mmcpart}  ${kernel_addr_r} ${bootenv}\0" \
        "importbootenv=echo Importing environment ...; env import -t ${scriptaddr} ${filesize}\0" \
